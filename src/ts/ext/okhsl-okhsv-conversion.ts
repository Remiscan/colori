import { oklab_to_xyzd65, xyzd65_to_srgblinear } from './w3-conversion.js';
...
function get_ST_max(a_: number,b_: number, cusp?: number[])
{
    if (!cusp)
    {
...
export function okhsl_to_oklab([h,s,l]: number[])
{
    if (l == 1)
    {
        return [1,0,0];
...
export function okhsv_to_oklab([h,s,v]: number[])
{
    if (v === 0) {
      return [0, 0, 0];
    }
