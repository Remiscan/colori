...
function CIEDE2000([l1, a1, b1], [l2, a2, b2]) {
    ...
    const T = 1 - 0.17 * Math.cos((Math.PI / 180) * (mhh - 30))
        ...
        - 0.20 * Math.cos((Math.PI / 180) * (4 * mhh - 63)), dTH = 30 * Math.exp(-1 * ((mhh - 275) / 25) ** 2), RC = 2 * Math.sqrt(mCC ** 7 / (mCC ** 7 + 25 ** 7)), SL = 1 + (0.015 * (mL - 50) ** 2) / Math.sqrt(20 + (mL - 50) ** 2), SC = 1 + 0.045 * mCC, SH = 1 + 0.015 * mCC * T, RT = -1 * Math.sin((Math.PI / 180) * (2 * dTH)) * RC;
    return Math.sqrt((dL / SL) ** 2
        + (dC / SC) ** 2
        + (dH / SH) ** 2
        + RT * (dC / SC) * (dH / SH));
}
...
function gamutIntersection(a, b, L1, C1, L0) {
    ...
    if (((L1 - L0) * Ccusp - (Lcusp - L0) * C1) <= 0) {
        ...
        const k_l = 0.3963377774 * a + 0.2158037573 * b, k_m = -0.1055613458 * a - 0.0638541728 * b, k_s = -0.0894841775 * a - 1.2914855480 * b;
...
class Couleur {
    ...
    static convert(startSpaceID, endSpaceID, values) {
        if ((typeof startSpaceID === typeof endSpaceID && startSpaceID === endSpaceID)
